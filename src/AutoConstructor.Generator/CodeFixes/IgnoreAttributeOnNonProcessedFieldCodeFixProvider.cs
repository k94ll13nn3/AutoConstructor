...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(IgnoreAttributeOnNonProcessedFieldCodeFixProvider)), Shared]
    public class IgnoreAttributeOnNonProcessedFieldCodeFixProvider : CodeFixProvider
    {
        public sealed override ImmutableArray<string> FixableDiagnosticIds => ImmutableArray.Create(IgnoreAttributeOnNonProcessedFieldAnalyzer.DiagnosticId);
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            if (declarationList is not null)
            {
                context.RegisterCodeFix(
                    CodeAction.Create(
                        title: $"Remove {Source.IgnoreAttributeFullName}",
                        createChangedDocument: c => RemoveAttributeAsync(context.Document, declarationList, declaration, c),
                        equivalenceKey: $"Remove {Source.IgnoreAttributeFullName}"),
                    diagnostic);
            }
        }
